...
export const getTwitchFollowerCount = async (account: ConnectedAccount): Promise<number> => {
  const isValidCredentials = credentials !== null && credentials.access_token_age + credentials.expires_in > Date.now()
  if (credentials == null || !isValidCredentials) {
    ...
    const { data: twitchCredentials } = await twitchOAuthAPI.post<TwitchCredentials>('/token', parameters)
    credentials = {
      ...twitchCredentials,
      access_token_age: Date.now()
    }
  }
  const { data: twitchData } = await twitchAPI.get<TwitchGetUsersFollowsResponse>('/users/follows', {
    headers: {
