...
const invitations: DiscordCommand = {
  ...
  execute: async (interaction) => {
    ...
    const memberSingularPlural = invitationsCount === 1 ? 'member' : 'members'
    await interaction.reply({
      content: `${userMention(interaction.user.id)} You invited ${bold(invitationsCount.toString())} ${memberSingularPlural}.`
    })
  }
}
